...
import { FileBarChart2, ListTodo, Download, Clock, AlertCircle, ShieldCheck } from 'lucide-react'
...
type Tab = 'requests' | 'sla' | 'tasks' | 'export' | 'scheduled'
...
export default async function ReportsPage({
  ...
  const tab: Tab = rawTab && ['requests','sla','tasks','export','scheduled'].includes(rawTab)
    ? rawTab
    : defaultTab
  ...
  // Parallel data fetching — only fetch what the active tab needs
  const [
    requestData,
    taskData,
    { data: scheduledReports = [] },
  ] = await Promise.all([
    (tab === 'requests' || tab === 'sla') && hasRequests ? getAnalytics(period) : Promise.resolve(null),
    tab === 'tasks'    && hasTasks    ? getTaskAnalytics(period) : Promise.resolve(null),
    getScheduledReports(),
  ])
      ...
      <div className="flex border-b border-border gap-1">
        {TABS.filter((t) => t.show).map((t) => {
          ...
          return (
            <Link
              key={t.id}
              href={`?tab=${t.id}${showPeriod || t.id === 'requests' || t.id === 'tasks' ? `&period=${period}` : ''}`}
